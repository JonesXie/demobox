...
$(function(){
  ...
  var encoders=[],encoder_max,encoder_min,_H,_S,_V,_L;
        ...
        if(res.faces.length>0){
          ...
          if(_dark == "None"){
            if(Math.abs(_rotates)<10){
              ...
              nose_tip = res.faces[0].landmark['contour_chin'].y;
  ...
  function _newc(_cw,_ch,dada,_leye,_reye,_mouth,_ct,_cl,_srcs,_rotates,_eyebrow,_newsrc,_faceeye) {
    ...
    //找到所有坐标
    var _ytotal,_xtotal,_xfor,_yfor,_xtol = [],_ytol = [];
    function _coordinate(dada) {
      ...
      _facem=[];
      for(var r = _xtotal;r< _xfor+1;r++){
        for(var k = _ytotal-1;k < _yfor+1;k++){
          _facem.push({"x":r,"y":k});
        }
      }
    }
    ...
    function _faceMain(dada,canvasDatas,facepoints,lefteyepoints,righteyepoints,mouthpoints) {
      ...
      //打点  原点开始 右下角结束描绘
      var facepoint_X = [],leye_X=[],reye_X=[],mouth_X=[],len,min,max,alpha=0.7,
    ...
    function drawing() {
      ...
      img.onload=function(){
        source = img;
        ctx.drawImage(source, _xtotal, _ytotal,_xfor-_xtotal,_yfor-_ytotal);
  ...
  function _loadImg() {
    ...
    function drawing(n){
      if(n<len){
        ...
        img.src=_loadDate[n];
        img.onload=function(){
          var _rw = c.width*0.78;
          var _rh = c.height*0.76;
          var _rwf = c.width*0.11;
          var _rwt = c.width*0.05;
          var _lwc = (c.height - _rh - _rwt)/2;
          var _lwt = (_rwt + _rh + _lwc*0.1);
          var _lwf = (_rwf + _lwc*1.05);
          var _lwk = (c.width - _lwf)*0.95;
          var _lwg = (_lwc*0.2 + _lwc);
          ...
          if(n===0){
            ...
            ctx.drawImage(img,_rwf,_lwt,_lwc,_lwc);
          }
          else if(n===4){
            ctx.drawImage(img,_lwf,_gt,_lwk,_lwg);
          }
          else if(n===5){
            ctx.drawImage(img,_rwf,_lwt+_lwc*1.1,_rw,_lwc/2);
          }
          else if(n===6){
            ctx.drawImage(img,_lwc*0.2,_lwc*0.15,_lwc,_lwc);
          }
          drawing(n+1);
        }
      }else{
                ...
                if(_newsrc){
                  if(_opens){
                    if(_srcs){
                      _loadDate=[];
                      _loadBase=[];
                      _loadDate.push('images/blue1.jpg',_newsrc[0],_ba_src[0],'images/ss1.png','images/ss2.png','images/rss3.png',_chinas);
                      _loadImg();
                    }
                    else{
                      alert('没有选择国家')
